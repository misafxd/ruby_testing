...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_class) { described_class.new('water') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_class.type).to be('water')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_class.ounces).to be(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_class) { described_class.new('coffee') }

      # remove the 'x' before running this test
      it 'is full' do
        expect(my_class).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_drink) { described_class.new('coffee', 9) }
      # remove the 'x' before running this test
      it 'is not full' do
