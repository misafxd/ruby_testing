...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:game) { described_class.new(0, 9, random_number, 7) }
    ...
    context 'when guess and random_number are not equal' do
      let(:random_number) { double('different_random_number', value: 2)}
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        updated_min = low_guess_game.min
        expect(updated_min).to eq(5)
      end

      it 'does not update max' do
        not_updated_max = low_guess_game.max
        expect(not_updated_max).to eq(9)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        not_updated_min = high_guess_game.min
        expect(not_updated_min).to eq(0)
      end

      it 'updates max to 8' do
        updated_max = high_guess_game.max
        expect(updated_max).to eq(8)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        updated_min = edge_cases.min
        expect(updated_min).to eq(8)
      end

      it 'does not update max' do
        not_updated_max = edge_cases.max
        expect(not_updated_max).to eq(8)
      end
    end
  end
end
